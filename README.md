...
|--------------------------------|----------------------------------|------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `-allowfrom`                   | `SP_ALLOWFROM`                   | `127.0.0.1/32`         | Specifies the IP addresses, networks (CIDR notation) or hostnames (comma-separated) of clients allowed to connect to the proxy. Prefix lengths are accepted for IPv4 and IPv6 addresses (e.g. `fdd0:0:0:4::1/64`); they are stripped when evaluating a literal address. The default value is `127.0.0.1/32` which limits access to localhost. To allow all IPv4 addresses, set `-allowfrom=0.0.0.0/0`. Alternatively, hostnames can be set, for example `-allowfrom=traefik` or `-allowfrom=traefik,dozzle`. Please remember that socket-proxy should never be exposed to a public network, regardless of this extra security layer. |
...
1.12 - fix parsing of IPv6/IPv4 addresses with prefix lengths in `-allowfrom`; allow CIDR-style entries without raising parse errors
